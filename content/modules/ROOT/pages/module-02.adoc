...
We need something to break with SELinux, so we will setup a basic web server. Run the following commands to install the package:
[source,bash,role="execute",subs=attributes+]
----
...
This seems like the right event, but the readability can be improved. Let's install the tool called `sealert` which helps take SELinx Logs and then help with suggestions on how to fix it.
[source,bash,role="execute",subs=attributes+]
----
sudo dnf install setroubleshoot-server -y
----

Let's try using the `sealert` command.:
[source,bash,role="execute",subs=attributes+]
----
...
.....
[rhel@rhel ~]$ sudo restorecon -Rv /var/www/html
